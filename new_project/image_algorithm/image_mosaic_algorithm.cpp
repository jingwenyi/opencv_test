...
void Image_algorithm::Image_fast_mosaic_algorithm2(cv::Mat &src_image1, cv::Mat &src_image2, cv::Point2i &distance)
{
	...
	cv::Point2i image1_sample_size(image1_gray.cols / 10, image1_gray.rows / 2);
...
void Image_algorithm::Image_fast_mosaic_algorithm3(cv::Mat &src_image1, cv::Mat &src_image2, cv::Point2i &distance)
{
	//需要先转换成灰度图像
	cv::Mat image1_gray, image2_gray;
	cv::cvtColor(src_image1, image1_gray, CV_RGB2GRAY);
...
void Image_algorithm::Image_fast_mosaic_algorithm4(cv::Mat &src_image1, cv::Mat &src_image2, cv::Point2i &distance)
{
	//需要先转换成灰度图像
	cv::Mat image1_gray, image2_gray;
	cv::cvtColor(src_image1, image1_gray, CV_RGB2GRAY);
	...
	int diff_x;
	//Get_sample_size_up_down(image_size, image1_sample_size, diff_x);
	diff_x = image_size.x * 2 / 3;
	image1_sample_size.x = image_size.x / 6;
	image1_sample_size.y = image_size.y / 8;
